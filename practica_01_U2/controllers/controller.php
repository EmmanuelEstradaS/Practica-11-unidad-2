...
class MvcController
{
	...
	###################3###############################################################3#######3############# aqui empieza practica 01_U1 ####################################
####################################################################################
...
public function registroUsuarioController()
	{
		...
		{
			$datosController = array( "name"=>$_POST["txtName"],
									  "username"=>$_POST["txtUsername"],
									  "password"=>$_POST["txtPassword"]);
									  

			$respuesta = Datos::registroUsuarioModel($datosController, "user");
...
#####################################################################################
	...
	public function vistaProblemasController()
	{
		 
			$arrayRespuesta = Datos::vistaProblemasModel("tipo_problema");
			echo '
				<table id="example1" class="table table-bordered table-striped">
					<thead>
						<tr>
			...
			echo'
					...
					<tr>
						<td>'.$item["id_tipo_problema"].'</td>
						<td>'.$item["nombre"].'</td>'; 
							if($_SESSION["id_tipo_usuario"]==1) 
							{ 
								echo '
								<td> <a title="Editar" href="index.php?action=editar_problema&id='.$item["id_tipo_problema"].'"> 
							        <button class="btn btn-info"><i class="fa fa-edit"></i> Editar</button>
							     </a> 
								</td>
								<td> <button title="Borrar" onClick="borrar('.$item["id_tipo_problema"].');" class="btn btn-danger" title= "Eliminar problema"> 
								<i class="fa fa-trash"></i> Eliminar </button></center> </td>
								</tr> ';
							}
			}
	          ...
	          swal("Ingrese su contraseña:", {
	            content: "input",
		          ...
		          .then((value) => 
		          {
		              if (value==password) 
		              {
		                swal("Contraseña correcta", "Tipo problema eliminado", "success");
		                window.location.href = "index.php?action=problemas&idBorrar="+id;
		              }
		              else
		              {
		                swal("Contraseña Incorrecta", "Intente de nuevo", "error");
...
public function registroProblemaController()
	{
		if(isset($_POST["btnEnviar"]))
		{
			$datosController = array( "nombre"=>$_POST["txtNombre"]);

			$respuesta = Datos::registroProblemaModel($datosController, "tipo_problema");
			if($respuesta == "success")
			{
				 
				echo '<script type="text/javascript">
						alert("Problema Agregada Exitosamente!");
					 </script>';

					 echo '<script type="text/javascript">
						window.location.href = "index.php?action=problemas";
					</script>';	
	...
	public function editarTipoProblemaController()
	{
		if(isset($_GET["id"]))
		{
			$idProblema = $_GET["id"];
			$respuesta = Datos::editarTipoProblemaModel($idProblema,"tipo_problema");
			return $respuesta;
		}
	}

	public function editarProblemaController()
	{
		if(isset($_GET["id"]))
		{
			$idCarrera = $_GET["id"];
	...
	public function actualizarProblemaController()
	{
		if(isset($_POST["btnEnviar"]))
		{
			$datosController = array( "nombre"=>$_POST["txtNombre"],
									  "id_tipo_problema"=>$_GET["id"]);

			$respuesta = Datos::actualizarProblemaModel($datosController, "tipo_problema");
			if($respuesta == "success")
			{
				 
				echo '<script type="text/javascript">
						alert("Problema Actualizado Exitosamente!");
					 </script>';
	...
	public function borrarProblemasController()
	{
		if(isset($_GET["idBorrar"]))
		{
			$respuesta = Datos::borrarProblemaModel($_GET["idBorrar"],"tipo_problema");
			if($respuesta == "success")
			{
				echo "<script>
						window.location='index.php?action=problemas'
					</script>";
			}
			else
			{
